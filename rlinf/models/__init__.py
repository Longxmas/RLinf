...
def _register_builtin_models():
    ...
    def _build_vvla(cfg: DictConfig, torch_dtype):
        from rlinf.models.embodiment.vvla import get_model

        return get_model(cfg, torch_dtype)

    def _build_vvla_gr00t(cfg: DictConfig, torch_dtype):
        from rlinf.models.embodiment.vvla.vvla_gr00t_action_model import get_model

        return get_model(cfg, torch_dtype)


    def _build_vvla_openvla_oft(cfg: DictConfig, torch_dtype):
        from rlinf.models.embodiment.vvla.vvla_openvla_oft_action_model import get_model

        return get_model(cfg, torch_dtype)

    def _build_vvla_lingbotvla(cfg: DictConfig, torch_dtype):
        from rlinf.models.embodiment.vvla.vvla_lingbotvla_action_model import get_model
    ...
    register_model(
        "vvla_openvla_oft",
        _build_vvla_openvla_oft,
        category="embodied",
        force=True,
    )
    register_model(
        "vvla_lingbotvla",
        _build_vvla_lingbotvla,
        category="embodied",
        force=True,
    )
    register_model(
